...
* @returns {Array} 插件列表
*/
export function getPluginsInfo(plugin) {
...
*/
export async function getAllPluginConfig() {
...
async function deconstructView(view, yaml, plugin) {
    let viewData = [];
    for (let config of view) {
        ...
        if (config.type === 'view' && config.api) {
            ...
            try {
                ...
                logger.error('远程视图获取失败：', error.message);
                continue;
            }
            try {
                // 将远程视图转变为Yaml对象 
...
async function getPluginView(plugin) {
...
* @returns {object} 配置信息
*/
export async function getPluginConfig(plugin) {
    const plugins = [...getPluginsList(), ...await getNpmPluginsList()];
...
*/
export async function setPluginConfig(plugin, file, key, value) {
    const plugins = [...getPluginsList(), ...await getNpmPluginsList()];
    if (!plugins.includes(plugin)) {
        return;
    }
