...
start()
// 创建面板管理账号
export const addUser = karin.command(/^#(添加|创建)面板管理账号/, async (e) => {
  // 判断是否群聊
  if (!e.isPrivate) {
    return e.reply('请私聊发送') !== undefined
  }
  // 获取消息中的密码
  let password = e.msg.replace(/^#(添加|创建)面板管理账号/, '').replace(/[\s\r\n]+/g, '')
  // 检查账号状态
...
// 修改面板管理账号密码
export const changePassword = karin.command(/^#(重置|修改)面板管理密码/, async (e) => {
  // 判断是否群聊
  if (!e.isPrivate) {
    return e.reply('请私聊发送') !== undefined
  }
  // 获取消息中的密码
  const password = e.msg.replace(/^#(重置|修改)面板管理密码/, '').replace(/[\s\r\n]+/g, '')
  // 检查账号状态
  if (UserManager.checkUser(e.user_id)) {
    if (password) {
      UserManager.changePassword(e.user_id, crypto.createHash('md5').update(password).digest('hex'))
