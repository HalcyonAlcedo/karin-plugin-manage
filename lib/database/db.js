...
const searchData = async (pattern, page, count) => {
  ...
  const redisMode = Config.Config.redis && db.db?.adapter?.client?.scan
  if (redisMode) {
    let cursor = '0'

    // 获取所有匹配的键
    do {
      const reply = await db.db.adapter.client.scan(cursor, 'MATCH', pattern, 'COUNT', 100)
      cursor = reply.cursor
      // 筛选匹配模式的键
