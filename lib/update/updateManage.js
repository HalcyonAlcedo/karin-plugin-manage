...
import { dirPath, dirname } from '../../index.js'
...
const backupUncommittedFiles = async (backupPath) => {
  ...
  for (const file of files) {
    ...
    console.log(path.join(dirname, file))
    fs.mkdirSync(path.dirname(filePath), { recursive: true })
    fs.copyFileSync(path.join(dirname, file), filePath)
  }
}
...
const pullUpdates = async (forceUpdate = false) => {
  ...
  try {
    ...
    if (forceUpdate && hasUncommittedChanges) {
      ...
      const backupPath = `${dirPath}/backups/${formatDate(new Date())}`;
