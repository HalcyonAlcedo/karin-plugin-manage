...
import { config as Cfg } from '../imports/index.js'
    ...
    switch (_data.type) {
      case 'web':
        if (_data.path) {
          ...
          if (query.html) {
            ws.send(JSON.stringify({ type: 'web', command: 'redirect', path: apiPath, target: query.html.startsWith('/') ? query.html.slice(1) : query.html, echo }))
            return
          }
          if (Config.debug) {
          ...
          if (_data.command === 'get') {
            axios.get(`http://localhost:${Config.port}/${apiPath}`, {
              ...
              ws.send(JSON.stringify({ type: 'web', state: 'error', error: { status: err.response?.status || 500, message: err.response?.statusText || err.message }, echo }))
            })
          } else {
          ...
          ws.send(JSON.stringify({ type: 'web', state: 'error', error: '错误的数据路径', echo }))
        }
        break
      case 'ws':
