...
export default async (fastify, options) => {
     ...
     */
    // 默认页面
    await fastify.get('/', async (_request, reply) => {
    ...
    // 获取快速登陆用户列表
    await fastify.post('/getLoginUserInfo', async (_request, reply) => {
    ...
    // 检查用户
    await fastify.post('/check-user', async (_request, reply) => {
        reply.send({ status: 'success', message: 'Check user success' });
    });
    // 获取面板用户列表
    await fastify.post('/userList', async (_request, reply) => {
