import { Bot, segment, redis } from '#Karin'
...
export default async (fastify, options) => {
  ...
  await fastify.post('/register', async (request, reply) => {
    ...
    try {
      ...
      // 添加用户
      UserManager.addUser(username, password, ['^/user/(?!register$|change-permissions$).*$']); // 默认只有/user的访问权限
  ...
  await fastify.post('/quickLogin', async (request, reply) => {
    ...
    for (let uid in Bot.adapter) {
      if (uid === bot.toString()) {
        ...
        try {
          const otpBot = Bot.adapter[parseInt(uid)]
          await otpBot.SendMessage(KarinContact.private(parseInt(qq)), [segment.text(`收到快速登陆请求：${authCode}`)])
          await redis.set(`karin-plugin-manage:user:otp`, authCode, { EX: 180 });
