...
const parseLogData = (logData, numRecords = null, filterLevel = '') => {
  ...
  if (Array.isArray(numRecords) && numRecords.length === 2) {
    const start = Math.max(0, records.length - numRecords[0])
    return records.slice(start, start + numRecords[1])
  }

  if (typeof numRecords === 'object' && numRecords !== null) {
    const { number, index } = numRecords
    if (typeof number === 'number' && typeof index === 'number') {
      const start = Math.max(0, records.length - index)
      return records.slice(start, start + number)
    }
  }
