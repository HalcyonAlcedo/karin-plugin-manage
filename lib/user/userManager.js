...
class UserManager {
  ...
  async login(username, password, remember) {
    ...
    const token = jwt.sign({ username, routes: user.routes }, this.secretKey, remember ? undefined : { expiresIn: '1h' });
  ...
  async setToken(username, token, remember) {
    try {
      await db.set(`karin-plugin-manage:user:${username}:token`, token, remember ? undefined : { EX: 60 * 60 })
    } catch (error) {
