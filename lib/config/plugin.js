...
const getPluginView = (plugin) => {
  ...
  return { config: pluginConfigData, view }
}
...
const GetAllPluginWidgets = async () => {
  ...
  let widgets = []
  for (const plugin of plugins) {
    const widgetPath = `plugins/${plugin}/config/widgets/`
    if (fs.existsSync(widgetPath)) {
      ...
      for (const widget of widgetFiles) {
        let data
        try {
          data = await (await import(`../../../../plugins/${plugin}/config/widgets/${path.parse(widget).name}.js`)).default()
        } catch (err) {
          data = undefined
        }
        widgets.push({
