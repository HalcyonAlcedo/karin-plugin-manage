import { YamlEditor, logger, common } from '#Karin'
import fs from 'fs'
...
const getPluginAssociated = (view, plugin, file) => {
  let associated = []
  for (const config of view) {
    if (config.type === 'group' && config.part) {
      associated = [...associated, ...getPluginAssociated(config.part, file)]
...
const GetAllPluginWidgets = async () => {
  const plugins = getPluginsList()
  let widgets = []
  for (const plugin of plugins) {
...
const GetPluginWidgets = async (plugin) => {
  const plugins = getPluginsList()
  if (!plugins.includes(plugin)) {
    return []
  }
