...
视图文件中可以加载远程视图实现动态视图功能，如果需要设置远程视图需要将`type`设置为`view`，并添加`api`参数，当api参数为链接时远程视图将从链接获取（get方法获取）视图数据，如果为字符串时则使用视图所应用的插件导出的api获取视图数据，视图数据要求返回yaml格式的字符串内容。

远程视图将被视为`group`加载到页面中，远程视图数据会被编码为`part`数据。
