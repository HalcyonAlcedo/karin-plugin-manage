...
import { getBots, common } from '@plugin/imports'
...
export default async (fastify: any, options: any) => {
  ...
  // 用户注册
  await fastify.post('/register', async (request: any, reply: any) => {
  ...
  // 用户登陆
  await fastify.post('/login', async (request: any, reply: any) => {
  ...
  // 快速登陆
  await fastify.post('/quickLogin', async (request: any, reply: any) => {
    // 获取机器人列表
    ...
    if (otp && bot) {
      try {
        ...
        return reply.status(401).send({ status: 'error', message: error?.message || error })
      }
    }

    for (const uid in getBots()) {
      // 处理加密qq
      let _bot: string | undefined
      if (uid === bot.toString()) {
        _bot = bot
  ...
  // 获取快速登陆用户列表
  await fastify.post('/getLoginUserInfo', async (_request: any, reply: any) => {
  ...
  // 更新 Token 时间
  await fastify.post('/refresh-token', async (request: any, reply: any) => {
  ...
  // 用户注销
  await fastify.post('/logout', async (request: any, reply: any) => {
  ...
  // 修改密码
  await fastify.post('/change-password', async (request: any, reply: any) => {
  ...
  // 修改用户权限
  await fastify.post('/change-permissions', async (request: any, reply: any) => {
    ...
    try {
      try {
        ...
        if (error instanceof ReferenceError) {
          return reply.send({ status: 'failed', message: error.message })
        } else {
          return reply.send({ status: 'failed', message: error })
        }
      }
    } catch (error:any) {
      return reply.status(400).send({ status: 'error', message: error?.message || error })
  ...
  // 检查用户
  await fastify.post('/check-user', async (_request: any, reply: any) => {
    reply.send({ status: 'success', message: 'Check user success' })
  })

  // 获取面板用户列表
  await fastify.post('/userList', async (_request: any, reply: any) => {
