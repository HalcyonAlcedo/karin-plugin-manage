...
export default async (fastify: FastifyInstance) => {
  ...
  fastify.post('/login', async (request, reply) => {
    const { username, password, remember } = request.body as { username: string, password: string, remember?: boolean }
    try {
  ...
  fastify.post('/quickLogin', async (request, reply) => {
    // 获取机器人列表
    const { bot, qq, otp } = request.body as { bot: string, qq: string, otp?: string }
