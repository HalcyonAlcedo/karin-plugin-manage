import { getPluginConfig, getAllPluginConfig, getPluginsList, setPluginConfig, getExamplePluginsList } from '@plugin/core/config'
export default async (fastify:any) => {
  // 获取插件列表
  await fastify.post('/GetPluginList', async (_request:any, reply:any) => {
    return reply.send({
      status: 'success',
      data: getPluginsList()
    })
  ...
  // 获取单插件列表
  await fastify.post('/GetExamplePluginList', async (_request:any, reply:any) => {
    return reply.send({
      status: 'success',
  ...
  await fastify.post('/GetAllPluginConfig', async (_request:any, reply:any) => {
  ...
  // 获取插件配置
  await fastify.post('/GetPluginConfig', async (request:any, reply:any) => {
  ...
  // 设置插件配置
  await fastify.post('/SetPluginConfig', async (request:any, reply:any) => {
